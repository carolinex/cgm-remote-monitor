      ...
      api().update(query, doc, { upsert: true }, function (err, updateResults) {
        ...
        if (!err) {
          if (updateResults.result.upserted) {
            ...
            console.log('updateResults', updateResults.result.upserted[0]._id)
            //
            //FIREBASE DATA
            const USER_ID = process.env.USER_ID;
            if (USER_ID) {
              db.collection('readingsTwo')
                .doc(updateResults.result.upserted[0]._id).set({
                  date: doc.date,
                  sgv: doc.sgv,
                  direction: doc.direction,
                  userID: USER_ID,
                  _id: updateResults.result.upserted[0]._id,
                })
                .then(() => {
                  console.log('Firebase set readings: DONE.')
                }).catch((error) => {
                  console.log('Firebase set readings: ERROR.', error)
                });
          } else {
            console.error('USER_ID .ENV NOT DEFINED!')
          }
        }
          ctx.bus.emit('data-update', {
            type: 'entries'
