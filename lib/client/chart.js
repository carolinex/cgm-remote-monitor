...
function init (client, d3, $) {
  ...
  chart.yAxis2 = d3.axisLeft(yScale2)
    .tickFormat(d3.format('d'))
    .tickValues(tickValues);
  ...
  chart.update = function update (init) {
    ...
    var chartContainerRect = chartContainer[0].getBoundingClientRect() || 0;
    ...
    var focusHeight = chart.focusHeight = chartHeight * 1;
    ...
    if (widthChanged || (chart.prevChartHeight !== chartHeight)) {
      ...
      if (init) {
        ...
        chart.focus.select('.y').append('rect')
          .attr('width', 35)
          .attr('height', focusHeight)
          .attr('fill', '#fff');
