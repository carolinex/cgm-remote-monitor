...
function init (client, d3, $) {
  ...
  chart.update = function update (init) {
    ...
    $('#chartContainer').height(parseInt($(window).height())*0.3);
    ...
    var chartContainerRect = parseInt($(window).height())*0.3;//chartContainer[0].getBoundingClientRect() || 250;
    ...
    var focusHeight = chart.focusHeight = chartHeight - PADDING_BOTTOM;
    var contextHeight = chart.contextHeight = chartHeight * .3;
    chart.basalsHeight = focusHeight / 4;

    console.log(parseInt($(window).height())*0.3, focusHeight, chartHeight )
    ...
    if (widthChanged || (chart.prevChartHeight !== chartHeight)) {
      ...
      chart.yScale2.range([focusHeight, 0]);
      ...
      if (init) {
        ...
        chart.focus.select('.x')
          .attr('transform', 'translate(0,' + (focusHeight-(PADDING_BOTTOM*2)) + ')')
          .call(chart.xAxis);
