...
function init (client, d3, $) {
  ...
  var newRow;
  for(var i=0; i<48; i++){
    var dateSplit = new Date(sortedEntries[i].mills).toLocaleString('pt-BR').split(' ')[0];
    var dateFormatted = dateSplit;
    var row = '<li style="background:'+sortedEntries[i].color+'">';
    row += '<div class="bg-value">'+sortedEntries[i].mgdl+'</div>';
    row += '<div class="bg-time">'+dateFormatted+'</div>'
    row += '</li>';
    newRow = d3.select('#readings-list').append(row);
  }
  ...
  chart.update = function update (init) {
    ...
    chart.focus.selectAll('.focus-bg')
