...
function init (client, d3) {
  ...
  renderer.addFocusCircles = function addFocusCircles () {
    ...
    function focusCircleTooltip (d) {
      ...
      client.tooltip.html('<span><strong>' + translate('BG') + ':</strong> ' + client.sbx.scaleEntry(d) + '</span>' +
          (d.type === 'mbg' ? '<br/><span><strong>' + translate('Device') + ': </strong>' + d.device : '') + '</span>' +
          (d.type === 'forecast' && d.forecastType ? '<br/><span><strong>' + translate('Forecast Type') + ': </strong>' + d.forecastType : '') + '</span>' +
          (rawbgInfo.value ? '<br/><span><strong>' + translate('Raw BG') + ':</strong> ' + rawbgInfo.value : '') + '</span>' +
          (rawbgInfo.noise ? '<br/><span><strong>' + translate('Noise') + ':</strong> ' + rawbgInfo.noise : '') + '</span>' +
          '<br/><span><strong>' + translate('Time') + ':</strong> ' + client.formatTime(getOrAddDate(d)) + '</span>')
