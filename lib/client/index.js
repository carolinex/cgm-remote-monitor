...
$('#loadingMessageText').html('Connectando...');
...
client.loadLanguage = function loadLanguage (serverSettings, callback) {
  ...
  client.settings.language = 'br'
  client.settings.focusHours = 24;
...
client.load = function load (serverSettings, callback) {
  ...
  var chart
    ...
    , currentNotify
    , currentAnnouncement
    , alarmSound = 'ding.mp3'//'alarm.mp3'
    , urgentAlarmSound = 'nudge.mp3'//'alarm2.mp3'
  ...
  client.focusRangeMS = times.hours(24).msecs;
  $('.focus-range li[data-hours=12]').addClass('selected');
  ...
  client.bottomOfPills = function bottomOfPills () {
    ...
    return containerBottom;//Math.max(bottomOfPrimary, bottomOfMinorPills, bottomOfStatusPills);
  };
  ...
  /***
   * // Incluído para colocar a lista de BG
   */
  function createBGList(){
    var sortedEntries = JSON.parse(JSON.stringify(client.entries));
    sortedEntries = sortedEntries.sort((a,b) => {
      return b.mills - a.mills;
    });
    d3.selectAll('#readings-list li').remove();
    var maxSize = sortedEntries.length < 12*24 ? sortedEntries.length : 12*24;


    var newRow;
    for(var i=0; i<maxSize; i++){
      var dateSplit = new Date(sortedEntries[i].mills).toLocaleString('pt-BR').split(' ');
      var dateFormatted = dateSplit[0].split('/').slice(0,-1).join('/')+ " - " + dateSplit[1].split(':').slice(0,-1).join(':');
      
      newRow = d3.select('#readings-list').append('li');
      newRow.attr('style', 'background:'+sortedEntries[i].color);
      newRow.append('div')
        .attr('class', 'bg-value')
        .text(sortedEntries[i].mgdl);
      newRow.append('div')
        .attr('class', 'bg-time')
        .text(dateFormatted);
    }
  ...
  function brushed () {
    ...
    function updateHeader () {
      ...
      console.log(focusPoint, inRetroMode())
    ...
    var top = container.height()//(client.bottomOfPills() + 5);
    $('#chartContainer').css({top: 'auto', height: ($(window).height()*0.6) - top - 10 });//top: 'auto', height: ($(window).height()*0.85) - top - 10
  ...
  function prepareEntries () {
    ...
    client.entries = client.entries.concat(client.ddata.mbgs.map(function(obj) {
      return { mills: obj.mills, mgdl: obj.mgdl, color: '#444444', type: 'mbg', device: obj.device, callibration: true };
    }));
